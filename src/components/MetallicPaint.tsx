...
export default function MetallicPaint({
  ...
  function updateUniforms() {
    ...
    gl.uniform1f(uniforms.u_edge, fullParams.edge);
    gl.uniform1f(uniforms.u_patternBlur, fullParams.patternBlur);
    gl.uniform1f(uniforms.u_time, 0);
    gl.uniform1f(uniforms.u_patternScale, fullParams.patternScale);
    gl.uniform1f(uniforms.u_refraction, fullParams.refraction);
    gl.uniform1f(uniforms.u_liquid, fullParams.liquid);
  }
  ...
  useEffect(() => {
    ...
    function render(currentTime: number) {
      ...
      totalAnimationTime.current += deltaTime * fullParams.speed;
  ...
  useEffect(() => {
    ...
    if (!canvasEl || !gl || Object.keys(uniforms).length === 0) return;

    function resizeCanvas() {
      if (!canvasEl || !gl || Object.keys(uniforms).length === 0 || !imageData) return;
  ...
  useEffect(() => {
    if (!gl || Object.keys(uniforms).length === 0 || !imageData) return;
